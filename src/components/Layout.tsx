...
import { Calendar, LayoutDashboard, LogOut, Menu, Plus, X, Bell, Search, Settings } from 'lucide-react';
    ...
    <div className="min-h-screen flex" style={{ background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed inset-y-0 left-0 w-72 flex flex-col shadow-2xl" style={{ background: 'var(--bg-secondary)', borderRight: '1px solid var(--border-subtle)' }}>
            ...
            <div className="p-4" style={{ borderTop: '1px solid var(--border-subtle)' }}>
              <div className="flex items-center gap-3 p-3 rounded-xl mb-3" style={{ background: 'var(--bg-card)' }}>
                <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-white text-lg" style={{ background: 'linear-gradient(135deg,var(--accent),#ff9a0d)' }}>
                  {user?.name?.charAt(0) || 'U'}
                </div>
                <div className="overflow-hidden">
                  <p className="text-sm font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{user?.name}</p>
                  <p className="text-xs uppercase tracking-wider truncate" style={{ color: 'var(--text-muted)' }}>{user?.role}</p>
                </div>
              </div>
      ...
      <div className="hidden lg:flex lg:flex-shrink-0">
        <div className="flex flex-col w-72 h-screen sticky top-0" style={{ background: 'var(--bg-secondary)', borderRight: '1px solid var(--border-subtle)' }}>
          ...
          <div className="p-4" style={{ borderTop: '1px solid var(--border-subtle)' }}>
            <div className="flex items-center gap-3 p-3 rounded-xl mb-3" style={{ background: 'var(--bg-card)' }}>
              <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-white text-lg flex-shrink-0" style={{ background: 'linear-gradient(135deg,var(--accent),#ff9a0d)' }}>
                {user?.name?.charAt(0) || 'U'}
              </div>
              <div className="overflow-hidden">
                <p className="text-sm font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{user?.name}</p>
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <header className="glass-nav sticky top-0 z-30 flex items-center h-16 px-4 gap-3">
          ...
          <div className={cn(
            ...
            isSearchExpanded ? "w-56 sm:w-72 px-3" : "w-10 h-10 justify-center"
